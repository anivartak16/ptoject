...
export async function marketsFor(commodity, origin = [75.8577, 22.7196]) {
	const prices = await MarketPrice.find({ commodity }).sort({ date: -1 }).populate("market");
	const newest = new Map();
	prices.forEach((price) => {
		if (!newest.has(String(price.market._id))) newest.set(String(price.market._id), price);
	});
	const distance = ([longitude, latitude]) => {
		...
		const a = Math.sin(deltaLatitude / 2) ** 2 + Math.cos(origin[1] * radians) * Math.cos(latitude * radians) * Math.sin(deltaLongitude / 2) ** 2;
	...
	return base.map((row) => {
		const netScore = maxNet === minNet ? 35 : ((row.netPrice - minNet) / (maxNet - minNet)) * 35;
		const reviewScore = (row.reviewAverage / 5) * 20;
		const distanceScore = Math.max(0, 20 - (row.distanceKm / maxDistance) * 20);
		const costScore = maxCost === minCost ? 25 : Math.max(0, 25 - ((row.estimatedTransportCost - minCost) / (maxCost - minCost)) * 25);
		return {
			...row,
			recommendationScore: Math.round(netScore + reviewScore + distanceScore + costScore),
			recommendationReasons: [`Net price ₹${row.netPrice}/kg after ₹${row.estimatedTransportCost} transport`, `Mandi rating ${row.reviewAverage.toFixed(1)}/5 (${row.reviewCount} reviews)`, `Distance ${row.distanceKm} km`, `Transport estimate ₹${row.transportCostPerKm}/km`],
		};
	}).sort((a, b) => b.recommendationScore - a.recommendationScore).map((row, index) => ({ ...row, isRecommended: index === 0, isNearest: row.distanceKm === nearestDistance }));
}
